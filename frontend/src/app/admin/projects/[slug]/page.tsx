        ...
        const visit = (node: any) => {
            if (node.type === 'image') {
                if (!node.url || node.url === '' || node.url === '#') {
                    // Replace with valid transparent image to prevent console error
...
export default function ProjectEditor({ params }: { params: Promise<{ slug: string }> }) {
    ...
    // Add custom command without removing default image command
    const customCommands = commands.getCommands ? [...commands.getCommands(), uploadImageCommand] : [];

    useEffect(() => {
        if (!isNew) {
            fetch(`${API_BASE}/projects/${paramSlug}`)
                .then(res => res.json())
                .then(data => {
                    setTitle(data.title);
                    setSlug(data.slug);
    ...
    const handleSave = async () => {
        ...
        const url = isNew ? `${API_BASE}/projects` : `${API_BASE}/projects/${paramSlug}`;
    ...
    return (
        <div className="w-full h-full overflow-y-auto bg-gray-50 p-6 pb-20">
            <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <textarea
                            className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition h-20 resize-none"
                            value={summary}
